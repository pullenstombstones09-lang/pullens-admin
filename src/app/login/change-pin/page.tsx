  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#1A1A2E] font-[Inter,system-ui,sans-serif]">
      ...
      <div className="w-full max-w-[360px] relative z-10">
        ...
        <div className="rounded-2xl p-6 bg-white/[0.05] border border-white/[0.1] shadow-[0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-sm">
          <div className="text-center mb-5">
            ...
            <p className="text-white/40 text-xs mt-1" id="step-label">Choose a new 4-digit PIN</p>
          </div>
          ...
          <form action="/api/auth/change-pin" method="POST" id="change-form">
            <input type="hidden" name="name" value={name} />
            <input type="hidden" name="newPin" id="new-pin-input" value="" />
          </form>
          ...
          <div className="grid grid-cols-3 gap-2.5 mb-4">
            ...
            <button
              type="button"
              id="clear-btn"
              className="min-h-[56px] rounded-xl bg-white/5 hover:bg-white/10 text-white/40 text-xs font-semibold uppercase tracking-wider transition-all duration-100 select-none"
            >
              Clear
            </button>
            <button
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        (function() {
          var step = 'new'; // 'new' then 'confirm'
          var newPin = '';
          var confirmPin = '';
          var input = document.getElementById('new-pin-input');
          ...
          function currentPin() { return step === 'new' ? newPin : confirmPin; }
            ...
            btn.addEventListener('click', function() {
              ...
              if (step === 'new' && newPin.length < 4) {
                ...
                if (newPin.length === 4) {
                  setTimeout(function() {
                    step = 'confirm';
                    label.textContent = 'Confirm your new PIN';
                    resetDots();
                  }, 300);
                }
                ...
                if (confirmPin.length === 4) {
                  if (newPin === confirmPin) {
                    ...
                    setTimeout(function() {
                      ...
                      confirmPin = '';
                      step = 'new';
                      label.textContent = 'PINs did not match — try again';
                      resetDots();
                    }, 800);
                  }
                }
