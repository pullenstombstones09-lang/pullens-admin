...
| Allandale biometric | HikVision DS-K1T343MWX at `192.168.8.11`, MAC `88:de:39:3f:d1:6d`, admin password in `.env.local` (`HIKVISION_PW`). FW V4.39.80, ISAPI 2.0, 30 users enrolled (22 May 2026). |
...
**Mapping locked in.** 27 device users matched cleanly to PT codes by `legacy_code` = device `employeeNo`. 1 mislabelled slot fixed: device 9019 labelled "Albert Johannes Masindo" is actually PT018 Thabiso Msindo (display name on device still wrong, mapping correct in DB). 2 device users not on payroll (8999 Veshi Moodley, 9037 Vishal Singh, 9000 Annika) — left enrolled, just unmapped. 7 Allandale employees still need to be enrolled on the device with pre-assigned codes (printable list at `Desktop/Pullens_Biometric_Enrolment_List.docx`, generated by `scripts/make-enrolment-docx.mjs`).
...
5. Enrol the 7 Allandale staff using the codes in the printable docx (PT010=9025, PT011=9029, PT019=9020, PT036=9015, PT037=9011, PT038=9012; PT029 Fika is now Pinetown not Allandale).
6. Rename device user 9019 display from "Albert Johannes Masindo" → "Thabiso Msindo".
7. Register UI optional: add a site badge per row so the clerk knows which staff need manual entry vs which are biometric-driven.
